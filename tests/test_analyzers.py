...
class TestFundamentalAnalyzer:
    ...
    def test_analyze_returns_string(self, fundamental_analyzer, sample_coin_id, sample_coin_name):
        """Test that analyze returns a string."""
        result = fundamental_analyzer.analyze(sample_coin_id, sample_coin_name)
        assert isinstance(result, str)
        assert len(result) > 0
    
    def test_analyze_includes_market_metrics(self, fundamental_analyzer, sample_coin_id, sample_coin_name):
    ...
    def test_analyze_includes_supply_metrics(self, fundamental_analyzer, sample_coin_id, sample_coin_name):
        """Test that analysis includes supply metrics."""
        result = fundamental_analyzer.analyze(sample_coin_id, sample_coin_name)
        assert "Supply" in result or "supply" in result.lower()
    
    def test_analyze_handles_errors_gracefully(self, coin_repository, sample_coin_id, sample_coin_name):
...
class TestPriceAnalyzer:
    ...
    def test_analyze_returns_string(self, price_analyzer, sample_coin_id, sample_coin_name):
        """Test that analyze returns a string."""
        result = price_analyzer.analyze(sample_coin_id, sample_coin_name)
        assert isinstance(result, str)
        assert len(result) > 0
    
    def test_analyze_includes_price_data(self, price_analyzer, sample_coin_id, sample_coin_name):
    ...
    def test_analyze_includes_volatility(self, price_analyzer, sample_coin_id, sample_coin_name):
        """Test that analysis includes volatility assessment."""
        result = price_analyzer.analyze(sample_coin_id, sample_coin_name)
        assert "Volatility" in result or "volatility" in result.lower()
    
    def test_analyze_includes_support_resistance(self, price_analyzer, sample_coin_id, sample_coin_name):
    ...
    def test_analyze_includes_trend_analysis(self, price_analyzer, sample_coin_id, sample_coin_name):
        """Test that analysis includes trend analysis."""
        result = price_analyzer.analyze(sample_coin_id, sample_coin_name)
        assert any(term in result.lower() for term in ["trend", "uptrend", "downtrend", "momentum"])


class TestSentimentAnalyzer:
    """Test sentiment analyzer."""
    
    def test_analyze_returns_string(self, sentiment_analyzer, sample_coin_id, sample_coin_name):
        """Test that analyze returns a string."""
        result = sentiment_analyzer.analyze(sample_coin_id, sample_coin_name)
        assert isinstance(result, str)
        assert len(result) > 0
    
    def test_analyze_includes_sentiment_score(self, sentiment_analyzer, sample_coin_id, sample_coin_name):
    ...
    def test_analyze_includes_community_metrics(self, sentiment_analyzer, sample_coin_id, sample_coin_name):
        """Test that analysis includes community metrics."""
        result = sentiment_analyzer.analyze(sample_coin_id, sample_coin_name)
        # Check for social media mentions
        assert any(term in result.lower() for term in ["twitter", "reddit", "telegram", "community"])
    
    def test_analyze_includes_fear_greed_index(self, sentiment_analyzer, sample_coin_id, sample_coin_name):
        """Test that analysis includes Fear & Greed Index."""
        result = sentiment_analyzer.analyze(sample_coin_id, sample_coin_name)
        assert "Fear" in result or "Greed" in result or "fear" in result.lower() or "greed" in result.lower()


class TestTechnicalAnalyzer:
    """Test technical analyzer."""
    
    def test_analyze_returns_string(self, technical_analyzer, sample_coin_id, sample_coin_name):
        """Test that analyze returns a string."""
        result = technical_analyzer.analyze(sample_coin_id, sample_coin_name)
        assert isinstance(result, str)
        assert len(result) > 0
    
    def test_analyze_includes_technical_indicators(self, technical_analyzer, sample_coin_id, sample_coin_name):
    ...
    def test_analyze_handles_insufficient_data(self, coin_repository, sample_coin_id, sample_coin_name):
